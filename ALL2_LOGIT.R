...
dgf_lg <- function(ni, nj, gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07, u_0, u_1, w_0, w_1){
  # RVs set up
  x = matrix(99, nrow = ni, ncol = 4) 
  z = matrix(99, nrow = nj, ncol = 3) 
  y = matrix(99, nrow = ni*nj, ncol = 1) 
  t = matrix(99, nrow = ni*nj, ncol = 1) 
  pr = matrix(99, nrow = ni*nj, ncol = 1) 
  r = matrix(99, nrow = ni*nj, ncol = 1) 
  sim = matrix(99, nrow = ni*nj, ncol = 17)
  ...
  for (i in 1:ni){
    for(j in 1:nj){
      r[ind, 1] <- rnorm(1,0,1)
      t[ind, 1] = gamma00 + gamma01*x[i,1] + gamma02*z[j,1] + gamma03*x[i,2] + gamma04*z[j,2] + gamma05*x[i,3] + gamma06*z[j,3] + gamma07*x[i,4] + U[i,1] + U[i,2]*x[i,1] + W[j,1] + W[j,2]*z[j,1] + r[ind, 1]
      pr[ind, 1] = 1/(1+exp(t[ind, 1]))
      y[ind, 1] = rbinom(1, 1, pr[ind, 1])
      tmp <- c(i,j, y[ind, 1], t[ind, 1], pr[ind, 1], x[i,1],z[j,1],x[i,2],z[j,2],x[i,3],z[j,3],x[i,4],U[i,1],U[i,2],W[j,1],W[j,2],r[ind, 1])
      sim[ind, ] <- tmp
      colnames(sim) <- c("i", "j", "y", "t", "pr","x1", "z1", "x2", "z2", "x3", "z3", "x4", "u0", "u1", "w0", "w1", "r")
      ind <- ind + 1
    }
  }
...
alz_lg <- function(df, ni, nj){
  ...
  ### :::::: Set up :::::: ###
  {
    ...
    # lm_reml <- list()
    bms <- list()
    converge <- data.frame(ml1 = NA, ml2 = NA)
  }
  ...
  pri2 = c(set_prior('normal(1,10)', class = 'Intercept'),
           set_prior('normal(-2,10)', coef = 'x1',class='b'),
           set_prior('normal(2,10)', coef = 'z1',class='b'),
           set_prior('normal(.3,10)', coef = 'x2',class='b'),
           set_prior('normal(.2,10)', coef = 'z2',class='b'),
           ...
           set_prior('normal(1.5,10)', coef = 'z3',class='b'),
           set_prior('normal(1,10)', coef = 'x4',class='b'),
           set_prior('cauchy(0,5)', class = 'sd', group = 'i'),
           set_prior('cauchy(0,5)', class = 'sd', group = 'j'),
           set_prior('cauchy(0,5)', coef = 'Intercept', class = 'sd', group = 'i'),
           set_prior('cauchy(0,5)', coef = 'Intercept', class = 'sd', group = 'j'))
  ...
  # For NM
  lm_ml[[1]] <- glmer(y ~ x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (1|j), data = df, family = binomial(), 
                      control = glmerControl(optimizer="Nelder_Mead", optCtrl = list(maxfun = 100000)))
  lm_ml[[2]] <- glmer(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (0+x1|i) + (1|j) + (0+z1|j), data = df, family = binomial(), 
                      control = glmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=100000)))
  
  # For BOBYQA
  lm_ml[[3]] <- glmer(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (1|j), data = df, family = binomial(), 
                      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))
  lm_ml[[4]] <- glmer(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (0+x1|i) + (1|j) + (0+z1|j), data = df, family = binomial(), 
                      control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
  
  
  ### For Bayes model ###
  bms[[1]] <- brm(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (1|j), data = df, prior = pri1, 
                  family = bernoulli(), iter = 1000, thin = 10, chains = 4)
  bms[[2]] <- brm(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (0+x1|i) + (1|j) + (0+z1|j), data = df, 
                  prior = pri2, family = bernoulli(), iter = 1000, thin = 10, chains = 4)
  ...
  out_all <- list("out_b" = out_b, "ml_f" = ml_f, "ml_r1" = ml_r1, "ml_r2" = ml_r2, "converge" = converge, 
                  "lm_ml" = lm_ml, "bms" = bms)
...
perf_lg <- function(out_all, gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07,
                    u_0, u_1, w_0, w_1){
...
run_sim_lg <- function(n_rep, ni, nj, gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07,
                       u_0, u_1, w_0, w_1){
  ...
  for (h in 1:n_rep){
    ...
    df[[h]] <- as.data.frame(dgf_lg(ni = ni, nj = nj, gamma00 = gamma00, gamma01 = gamma01, gamma02 = gamma02, gamma03 = gamma03,
                                    gamma04 = gamma04, gamma05 = gamma05, gamma06 = gamma06, gamma07 = gamma07,
                                    u_0 = u_0 , u_1 = u_1, w_0 = w_0, w_1 = w_1))
    out_all[[h]] <- alz_lg(df = df[[h]], ni = ni, nj = nj)
    perf[[h]] <- perf_lg(out_all = out_all[[h]], gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07,
                         u_0, u_1, w_0, w_1)
  ...
  #saveRDS(perf, file = "perf_all2_lg100.rds")
  return(perf)
...
### RUN ###
out2lg <- run_sim_lg(n_rep = n_rep, ni = ni, nj = nj, gamma00 = gamma00, gamma01 = gamma01, gamma02 = gamma02, gamma03 = gamma03,
                     gamma04 = gamma04, gamma05 = gamma05, gamma06 = gamma06, gamma07 = gamma07,
                     u_0 = u_0 , u_1 = u_1, w_0 = w_0, w_1 = w_1)
...
#bias_est <- as.data.frame(sapply(out_lg, "[[", "bias"), row.names = rnames)
#ciw_est <- as.data.frame(sapply(out_lg, "[[", "ci_width"), row.names = rnames)
#rhat_each <- as.data.frame(sapply(out_lg, "[[", "rhat"), row.names = rnames)
#names(ciw_est) <- NULL


#rhat1_est <- sapply(out_lg, "[[", "rhat_p1")
#rhat2_est <- sapply(out_lg, "[[", "rhat_p2")
#rhat_est <- as.data.frame(rbind(rhat1_est, rhat2_est))

#saveRDS(bias_est, file = "bias_est2_100lg.rds")
#saveRDS(ciw_est, file = "ciw_est2_100lg.rds")
#saveRDS(rhat_est, file = "rhat_est2_100lg.rds")
#saveRDS(rhat_each, file = "rhat_each2_100lg.rds")
...
#my.file.rename(from = paste0("2lgout_", ni, "_", nj, ".rds"),
#               to = paste0("/staging/mhuang233/2lgout_", ni, "_", nj, ".rds"))

saveRDS(out2lg, file = paste0("cc", n_iter, "_", n_rep, "_2lg_", ni, "_", nj, ".rds"), compress = T)

# file.copy(from = paste0("cc", n_iter, "_", n_rep, "_2lg_", ni, "_", nj, ".rds"),
#             to = paste0("/staging/mhuang233/cc", n_iter, "_", n_rep, "_2lg_", ni, "_", nj, ".rds"))

# file.remove(paste0("cc", n_iter, "_", n_rep, "_2lg_", ni, "_", nj, ".rds"))
