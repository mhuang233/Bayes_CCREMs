...
dgf <- function(ni, nj, gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07, u_0, u_1, u_2, w_0, w_1, w_2){
  # RVs set up
  ...
  for (i in 1:ni){
    for(j in 1:nj){
      r[ind, 1] <- rnorm(1,0,1)
      t[ind, 1] = gamma00 + gamma01*x[i,1] + gamma02*z[j,1] + gamma03*x[i,2] + gamma04*z[j,2] + gamma05*x[i,3] + gamma06*z[j,3] + gamma07*x[i,4] + U[i,1] + U[i,2]*x[i,1] + U[i,3]*x[i,2] + W[j,1] + W[j,2]*z[j,1]+ W[j,3]*z[j,2] + r[ind, 1]
      pr[ind, 1] = 1/(1+exp(t[ind, 1]))
      y[ind, 1] = rbinom(1, 1, pr[ind, 1])
      tmp <- c(i,j,y[ind, 1],t[ind, 1], pr[ind, 1], x[i,1],z[j,1],x[i,2],z[j,2],x[i,3],z[j,3],x[i,4],U[i,1],U[i,2],U[i,3],W[j,1],W[j,2],W[j,3],r[ind, 1])
      sim[ind, ] <- tmp
      colnames(sim) <- c("i", "j", "y", "t", "pr","x1", "z1", "x2", "z2", "x3", "z3", "x4", "u0", "u1", "u2","w0", "w1", "w2","r")
      ind <- ind + 1
    }
  }
...
# TRY
#df <- dgf(ni, nj, gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07,
 #         u_0, u_1, u_2, w_0, w_1, w_2)
...
alz <- function(df, ni, nj){
  ...
  pri2 = c(
    ...
    set_prior('cauchy(0,5)', coef = 'Intercept', class = 'sd', group = 'i'),
    set_prior('cauchy(0,5)', coef = 'Intercept', class = 'sd', group = 'j'),
    set_prior('cauchy(0,5)', coef = 'x1', class = 'sd', group = 'i'),
    set_prior('cauchy(0,5)', coef = 'z1', class = 'sd', group = 'j'),
    set_prior('cauchy(0,5)', coef = 'x2', class = 'sd', group = 'i'),
    set_prior('cauchy(0,5)', coef = 'z2', class = 'sd', group = 'j'))
  ...
  lm_ml[[2]] <- glmer(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (0+x1|i) + (0+x2|i) + (1|j) + (0+z1|j)+ (0+z2|j), data = df,  family = binomial(),
                     control = glmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=100000)))
  ...
  lm_ml[[4]] <- glmer(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (0+x1|i) + (0+x2|i) + (1|j) + (0+z1|j)+ (0+z2|j), data = df, family = binomial(),
                     control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
  ...
  ### For Bayes model ###
  bms[[1]] <- brm(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (1|j), data = df, prior = pri1, family = bernoulli(),iter = 10000, thin = 10, chains = 4)
  bms[[2]] <- brm(y ~  x1 + z1 + x2 + z2 + x3 + z3 + x4 + (1|i) + (0+x1|i) + (0+x2|i) + (1|j) + (0+z1|j)+ (0+z2|j), data = df, prior = pri2, 
                  family = bernoulli(), iter = 10000, thin = 10, chains = 4)
...
run_sim <- function(n_rep, ni, nj, gamma00, gamma01, gamma02, gamma03, gamma04, gamma05, gamma06, gamma07,
  ...
  for (h in 1:n_rep){
    ...
    df[[h]] <- as.data.frame(dgf(ni = ni, nj = nj, gamma00 = gamma00, gamma01 = gamma01, gamma02 = gamma02, gamma03 = gamma03,
                                 ...
                                 u_0 = u_0 , u_1 = u_1 , u_2 = u_2, w_0 = w_0, w_1 = w_1, w_2 = w_2))
  ...
  # saveRDS(perf, file = "perf_all4_50lg.rds")
  return(perf)
...
### RUN ###
out4lg <- run_sim(n_rep = n_rep, ni = ni, nj = nj, gamma00 = gamma00, gamma01 = gamma01, gamma02 = gamma02, gamma03 = gamma03,
                    gamma04 = gamma04, gamma05 = gamma05, gamma06 = gamma06, gamma07 = gamma07,
                    u_0 = u_0 , u_1 = u_1 , u_2 = u_2, w_0 = w_0, w_1 = w_1, w_2 = w_2)
...
#saveRDS(bias_est, file = "bias_est4_50lg.rds")
#saveRDS(ciw_est, file = "ciw_est4_50lg.rds")
#saveRDS(rhat_est, file = "rhat_est4_50lg.rds")
#saveRDS(rhat_each, file = "rhat_each4_50lg.rds")


#saveRDS(out_list, file = paste0("4lgout_", ni, "_", nj, ".rds"), compress = T)
...
#my.file.rename(from = paste0("4lgout_", ni, "_", nj, ".rds"),
#               to = paste0("/staging/mhuang233/4lgout_", ni, "_", nj, ".rds"))

saveRDS(out4lg, file = paste0("cc", n_iter, "_", n_rep, "_4lg_", ni, "_", nj, ".rds"), compress = T)

# file.copy(from = paste0("cc", n_iter, "_", n_rep, "_4lg_", ni, "_", nj, ".rds"),
#          to = paste0("/staging/mhuang233/cc", n_iter, "_", n_rep, "_4lg_", ni, "_", nj, ".rds"))

# file.remove(paste0("cc", n_iter, "_", n_rep, "_4lg_", ni, "_", nj, ".rds"))
